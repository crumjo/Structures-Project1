/*****************************************************************
 Functions to read, write, and reverse a character pointer.
...
/*****************************************************************
 ...
 @param size the number of elements in the character array.
 @return int 0 if the code executed correctly, 1 otherwise.
 *****************************************************************/
int backwards ( char* buffer, int size )
{
    
    /** New char array for reversed contents of buffer. */
    char* rev_buffer = malloc(size * sizeof(char));
    if (rev_buffer == NULL) {
        fprintf(stderr, "\nMemory allocation failed.\n");
        return -1;
    }
    ...
    for (int i = 0; i < size; i++) {
        char x = buffer[(size - 1) - i];
        rev_buffer[i] = x;
    }
    
    rev_buffer[size] = '\0';
    strncpy(buffer, rev_buffer, size);
    free(rev_buffer);
